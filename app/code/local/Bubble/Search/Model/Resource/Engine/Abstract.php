...
abstract class Bubble_Search_Model_Resource_Engine_Abstract
{
    ...
    /**
     * Returns suggestions based on specified query.
     *
     * @param string $query
     * @return array
     */
    ...
    {
        if (!$this->_getHelper()->isSuggestEnabled()) {
            return array();
        }

        try {
            Varien_Profiler::start('BUBBLE_SEARCH_SUGGEST');
            $result = $this->_suggest($query);
            Varien_Profiler::stop('BUBBLE_SEARCH_SUGGEST');
