...
document.observe("dom:loaded", function() {
	...
	this.list.insert({ after : '<div class="more"><span id="more_button" class="more-button">'+ more +'</span></div>'});
 		...
 		this.button = jQuery('.more')[0];
			this.btn = jQuery('#more_button')[0];
    ...
    $$('.my-account .dashboard .box-title').each(function (elem) {
        elem.observe('click', function (e) {
            if ( e.target.hasClassName('box-title') ||e.target.hasClassName('title') ) {
    ...
    $$("#nav a").each(function(sliderLink) {
        ...
        if (sliderLink.next(0) !== undefined) {
			...
			var back_button = new Element('button', {'class': 'previous-category'}).update(back);
					
                     subcategoryHeader.insert({
                        top:back_button.wrap('div', {'class':'button-wrap'}) ,
		    	...
		    	bottom:this.innerHTML
                    ...
                    this.clonedSubmenuList.insert({
                        top: subcategoryHeader
                                        ...
                                        subcategoryHeader.insert({ after : new Element('li').update('<a href="' + sliderLink.href + '"><span>' + allproduct + '</span></a>') });
					
					var t =  jQuery('.button-wrap .previous-category')
                    jQuery('.button-wrap .previous-category')[ t.length-1].observe('click', function(e) {
					//	alert('click into');
                       if (last) {
                            diff = e.timeStamp - last
                        }
                        ...
                        $("nav-container").setStyle(transformValue);
						/*jQuery('.homepage_slideshow').css({'display':'block'});
						jQuery('.content_home_banner').css({'display':'block'});
						jQuery('.home-featured-mobile').css({'display':'block'}); */
						
                        sliderPosition = sliderPosition + document.body.offsetWidth;
                        setTimeout(function() { $$("#nav-container > ul:last-child")[0].remove(); $("nav-container").setStyle({'height' : 'auto'})  }, 350)
                    });
                ...
                $("nav-container").setStyle(transformValue);
				/*jQuery('.homepage_slideshow').css({'display':'none'});
				jQuery('.homepage_slideshow').css({'display':'none'});
				jQuery('.content_home_banner').css({'display':'none'});
				jQuery('.home-featured-mobile').css({'display':'none'});*/
	...
	/*Nav Blog*/	
	 $$("#nav_blog a").each(function(sliderLink) {
        if (sliderLink.next(0) !== undefined) {
            sliderLink.clonedSubmenuList = sliderLink.next(0);
			...
			var back_button = new Element('button', {'class': 'previous-category'}).update(back);
		    	...
		    	bottom:this.innerHTML
                    ...
                    subcategoryHeader.insert({ after : new Element('li').update('<a href="' + sliderLink.href + '"><span>'+ allpost +'</span></a>') });
...
var t = jQuery('.button-wrap .previous-category');
                    jQuery('.button-wrap .previous-category')[t.length-1].observe('click', function(e) {
                        ...
                        $("nav-container").setStyle(transformValue);
                        sliderPosition = sliderPosition + document.body.offsetWidth;
                        setTimeout(function() { $$("#nav-container > ul:last-child")[0].remove();var t=jQuery("#nav-container").children().length; $("nav-container").setStyle({'height' : $$("#nav-container > ul")[t-1].clientHeight+'px'})  }, 250)
                    });
                    new NoClickDelay(this.clonedSubmenuList);
                };

                $("nav-container").insert(this.clonedSubmenuList.setStyle({'width' : document.body.offsetWidth + 'px'}));
                 $('nav-container').setStyle({'height' : this.clonedSubmenuList.clientHeight+'px'});
	...
	*/
    ...
    var Carousel = Class.create({
       ...
       init: function () {
           ...
           this.resizeChilds();
           this.drawCounter();
/*	if($(this.carousel).className=="product-image")
		   {
				var zoomBtn = jQuery('#zoomIcon');
				var ListItem = jQuery('#product_addtocart_form .carousel-items li a');
				var getLink = jQuery(ListItem[0]).attr("href");
	...
	/*if($(this.carousel).className=="product-image")
					{
						...
						//var offset = (40/size)*100;
						//var realPer = -(-this.itemPos+offset)
						*/
...
/*if($(this.carousel).className=="product-image")
					{
						var size = jQuery(window).width();
						//var offset = -this.itemPos*100/size; */
...
function AddClassBodyLanguage(){
	
	jQuery("body").addClass(lang_code);
...
jQuery(document).ready(function ()  {
	AddClassBodyLanguage();	
	startReduce()
	jQuery.PictureSlides.init();
	var width = jQuery(window).width();
	
	if(width<480 && width>=200)	
		feature = new CodespotSwipe(jQuery(".products-grid"),{visibleElement:1,containerGap:40,elementGap:0});
	else 
		if(width>=480)	
			feature = new CodespotSwipe(jQuery(".products-grid"),{visibleElement:2,containerGap:40,elementGap:13});
		
	
	var pictureSlide = new CodespotSwipe(jQuery(".jcarousel-skin-tango"),{visibleElement:4,elementGap:10,navigation:true,containerGap:40,border:0});
	gallery = new CodespotSwipe(jQuery(".gallery"),{visibleElement:1,elementGap:0,navigation:true,showZoomIcon:true});
	var slideshow = new CodespotSwipe(jQuery(".blog-slider"),{visibleElement:1,elementGap:10,containerGap:20,border:0});
/*gallery2 = new CodespotSwipe(jQuery(".catalog-product-gallery .gallery"),{visibleElement:1,elementGap:10,navigation:true,showZoomIcon:true,containerGap:40}); */
	var account_toggle = new toggle("footer .footer_account h3","footer .footer_account ul");
	var storelink_toggle = new toggle("footer .footer_store_link h3","footer .footer_store_link ul");
	var about_toggle = new toggle("footer .about-store-mobile h3","footer .about-store-mobile p");
	
	var discount = new toggle(".checkout-cart-index #discount-coupon-form h2",".checkout-cart-index #discount-coupon-form .discount-form");
	var shipping = new toggle(".checkout-cart-index .shipping h2",".checkout-cart-index .shipping #shipping-zip-form")
	//var title = new toggle(".box-title",".box-content")
});
